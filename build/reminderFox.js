...
//ALARM	reminderfox.core.logMessageLevel("alarm: reminder date: " + newDate + "; alarmMinutes: " + alarmMinutes , reminderfox.consts.LOG_LEVEL_SUPER_FINE);  //TODO
...
//		"timeDiff: " + timeDifference + "\nlastAck: " + recentReminderOrTodo.alarmLastAcknowledge + "; Listname: " + listName , reminderfox.consts.LOG_LEVEL_SUPER_FINE);  //TODO
//	dump("\n\n**************\nProcess Alarm: " + recentReminderOrTodo.date + "\n" + recentReminderOrTodo.summary +"\n" +
//		"alarmMinutes; " + alarmMinutes + " --- alarmTime: " + alarmTime + " == currentTime: " + currentTime +
//		"timeDiff: " + timeDifference + "\nlastAck: " + recentReminderOrTodo.alarmLastAcknowledge + "; Listname: " + listName , reminderfox.consts.LOG_LEVEL_SUPER_FINE);  //TODO
//	reminderfox.core.logMessageLevel("\n\n**************\nProcess Alarm: " + new Date() + " -- " + recentReminderOrTodo.date + "\n" + recentReminderOrTodo.summary +"\n" +
//		"alarmMinutes; " + alarmMinutes + " --- alarmTime: " + alarmTime + " == currentTime: " + currentTime +
//		"timeDiff: " + timeDifference + "\nlastAck: " + recentReminderOrTodo.alarmLastAcknowledge + "; Listname: " + listName , reminderfox.consts.LOG_LEVEL_DEBUG);  //TODO
	if ( timeDifference < 0 ) {
		// check acknowledge
...
//ALARM		reminderfox.core.logMessageLevel( "alarm:set missed: " + missedAlarm, reminderfox.consts.LOG_LEVEL_SUPER_FINE);  //TODO
	}
...
//ALARM			reminderfox.core.logMessageLevel( "alarm:1 FOUND: " + found, reminderfox.consts.LOG_LEVEL_SUPER_FINE);  //TODO
			...
			if ( !found ) {
				// add him
//ALARM				reminderfox.core.logMessageLevel( "alarm: Setting alarm: " +recentReminderOrTodo.summary, reminderfox.consts.LOG_LEVEL_DEBUG);  //TODO
				...
				//reminderfox.core.logMessageLevel( "alarm: ADDED:  len= " + oldestWindow.reminderfox.overlay.alarmList.length + "; " +recentReminderOrTodo.summary + "; alarmId: " + alarmReminderId + "; timeOfAlarm: " + alarmTime, reminderfox.consts.LOG_LEVEL_DEBUG);  //TODO
...
//					}
					...
					var lblparent = document.getElementById("reminderfox-todaysRemindersBox").cloneNode(true);
					var lblparent2 = document.getElementById("reminderfox-upcomingRemindersBox").cloneNode(true);
					...
					if (!ignoreAlert) {
						...
						var newOptions = {
							todaysreminders: lblparent,
							upcomingreminders: lblparent2,
							alertTypeToShow: alertType
						};
						window.openDialog("chrome://reminderfox/content/alerts/reminderalert.xul", "window:alert2", "chrome,dialog=yes,titlebar=no,popup=yes", newOptions);

						// use this if you want to have a close button on the alert window
						// window.openDialog("chrome://reminderfox/content/alerts/reminderalert.xul", "window:alert2", "chrome,dialog=yes,titlebar=yes,popup=no", newOptions);
...
alert( "2")
       // store back file in reminderfox/profile directory 	
       var backupDir = reminderfox.util.ProfD_extend("reminderfox").append("backups");
       alert( "3" + backupDir)
       if ( !backupDir.exists() ) {	
       	alert( "4")
       		backupDir.create(backupDir.DIRECTORY_TYPE, 0775);
       }
       // get datestring for today in 01-11-2014-0514 format
       // datestring 
       alert( "5")
		var targetDate = new Date();
	 	var dateStr = targetDate.getFullYear() + "-" + targetDate.getMonth() + "-" + targetDate.getDate();
        
        var timeStr = targetDate.getHours() + targetDate.getMinutes();
       	alert( "DATESTR: " + dateStr + " -- " + timeStr);
       var todayDir = backupDir.append( "reminderfox-" + dateStr );
       	if ( !todayDir.exists() ) {	
       		alert( "NO EXIST" );
       		todayDir.create(todayDir.DIRECTORY_TYPE, 0775);
       		alert( "CREATED" );
			...
			alert( "COPYING TO" + "reminderfox-" + dateStr + "T" + timeStr);
			origFile.copyTo(todayDir, "reminderfox-" + dateStr + "T" + timeStr);
			alert( "DONE")
       		
       }
       else {
       	// today dir already exists
       	alert( "ALREADY EXIST")
       }
       ...
       // hmm how... would have to scan all files for that starting date prefix.  Or else don't record the time ?
       //
       // or create folders for every date and just write time-based file each time on startup
       // then don't worry about .bak, etc.
		...
		alert( "FAILED"  + e.name + ": " + e.description + ": " + e.number + ": " + e.message)
		...
		if (!processed) {
			...
			if (windowEnumerator.hasMoreElements()) {
				...
				if (actualAlarmTime > 3600000) {
								//  if alarm greater than an hour; don't add it; just let hourly process do it
				}
				else {
	...
	if (windowEnumerator.hasMoreElements()) {
		...
		if ( window != oldestWindow ) {
			...
			if ( !ignoreFirstRun ) {
				...
				// update the GCal OAuth2 token for all known&active user/accounts
				setTimeout(function () {rmFx_CalDAV_SyncActiveAccounts(null)}, 5000);
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	var xslFile = xThis.value; //  xThis = [object]

	if (isXPI == 'XPI') { // system templates
		var rmFxDir = reminderfox_prntPath();
	} else { // 'user' templates
		var rmFxDir = reminderfox.util.ProfD_extend('reminderfox');
	}

	rmFxDir.append(xslFile);
	var rmFxDirPath = rmFxDir.path;

	var orgFile = Components.classes["@mozilla.org/file/local;1"].createInstance(Components.interfaces.nsILocalFile);
	orgFile.initWithPath(rmFxDirPath);

	var pPath = reminderfox_prntPath();
	orgFile.copyTo(pPath, 'current.xsl');


	//--- Agenda printing if xslFile is 'Agenda.xsl'----------
	var remindersToday = null;
	var remindersUpcoming = null;
	var todos = "";
	var lists = "";  // XXX tdb

	if (xslFile == 'Agenda.xsl') {
		var todaysAndUpcomingReminders = reminderfox.overlay.getTodaysAndUpcomingReminders();

		var remindersToday = todaysAndUpcomingReminders.today;
		if (remindersToday == null) remindersToday = "";	// make sure it's not empty with "no today reminders"
		var remindersUpcoming = todaysAndUpcomingReminders.upcoming;
	...
	var xmlDoc = document.implementation.createDocument("", "", null);
		...
		var xmlGeneral = document.implementation.createDocument("", "", null);
		var generalElem = xmlGeneral.createElement("general");
		xmlDoc.appendChild(generalElem);

			var cDate = new Date();

			var attrElem = xmlGeneral.createElement('remindertype');
			attrElem.textContent= isReminder ? "reminder" : "todo's";
			generalElem.appendChild(attrElem);

			var attrElem = xmlGeneral.createElement('remindertitle');
			attrElem.textContent=  reminderfox.tabInfo.tabName;
			generalElem.appendChild(attrElem);

			var attrElem = xmlGeneral.createElement('date');
			attrElem.textContent= cDate.toLocaleDateString();
			generalElem.appendChild(attrElem);

			var attrElem = xmlGeneral.createElement('time');
			attrElem.textContent= cDate.toLocaleTimeString();
			generalElem.appendChild(attrElem);

			var attrElem = xmlGeneral.createElement('version');
			attrElem.textContent= "Reminderfox vers." 
				+ reminderfox.consts.MIGRATED_PREF_VERSION
				+ reminderfox.consts.SPECIAL_VERSION_DETAIL;
			generalElem.appendChild(attrElem);

			var attrElem = xmlGeneral.createElement('template');
			attrElem.textContent= rmFxDir.path;
			generalElem.appendChild(attrElem);

			var attrElem = xmlGeneral.createElement('xlsName');
			attrElem.textContent= xslFile;
			generalElem.appendChild(attrElem);

			var attrElem = xmlGeneral.createElement('app');
			attrElem.textContent= navigator.userAgent + " (" + navigator.language + ")";
			generalElem.appendChild(attrElem);


		var xmlReminders = document.implementation.createDocument("", "", null);
		var remindersElem = xmlReminders.createElement("reminders");


		// ---  view & print from  'List' -- it's not Agenda! ;)
		if (remindersToday == null)  {

			if (xslFile == 'Events(Selected).xsl') {
				// collect the selected row(s) on the List
				if (isReminder) {
					var treeitems = document.getElementById("treechildren").childNodes;
					var indices = getAllSelectedReminderIndices();
				} else {
					var treeitems = document.getElementById("todo_treechildren").childNodes;
					var indices = getAllSelectedTodoIndices();
				}
	
				for (var i = 0; i < indices.length; i++) {
					var row = treeitems[indices[i]].childNodes[0];
				...
				if (isReminder) {
					var treeitems = document.getElementById("treechildren").childNodes;
					var treeLength = treeitems.length;
				} else {
					var treeitems = document.getElementById("todo_treechildren").childNodes;
					var treeLength = treeitems.length;
				}
				...
				for (var i = 0; i < treeLength; i++) {
					var row = treeitems[i].childNodes[0];
						buildRow(isReminder, row, remindersElem);
				}
			}
		...
		// --- for Agenda
		if (remindersToday != null)  {
			create_element ('today', remindersToday, remindersElem);
		} // view & print for Agenda

		if (remindersUpcoming != null)  { // view & print for Agenda
			create_element ('upcoming', remindersUpcoming, remindersElem);
		} // view & print for Agenda

		if (todos != null)  { // view & print for Agenda
			create_element ('todos', todos, remindersElem);
		} 
		// --- for Agenda


		function buildRow (isReminder, row, remindersElem) {
			var id = row.getAttribute("idRef");

			if (isReminder) var thisEvent = reminderfox.core.getRemindersById(id);
			if (!isReminder) var thisEvent = reminderfox.core.getSpecificTodoById(id);

			var dateRef = row.getAttribute("dateRef");
			var actualDate = new Date(parseInt(dateRef));
			var actualSummary = row.childNodes[1].getAttribute("label");


			var eventElem = xmlReminders.createElement("event");
			var allDayEvent = false;
			var durationTime = 0;

			for (var attribute in thisEvent) {

				var attrValue = thisEvent[attribute];

				if (thisEvent[attribute] instanceof Date) {

					if (!((attribute == 'endDate') && (allDayEvent == true))) {
						var dateTime = attrValue.getTime()

						switch (attribute) {
						case 'date': 
							var dateString = reminderfox.date.getDateVariableString (thisEvent, actualDate);
							var shortDay = reminderfox.date.getDateVariable (thisEvent, actualDate, "[shortDay]");
							var timeString = reminderfox.date.getDateVariable (thisEvent, actualDate, "[time]");
							var dateRef =  actualDate.getTime();
							break;
						...
						default: 
							var dateString = reminderfox.date.getDateVariableString (thisEvent, attrValue);
							var timeString = reminderfox.date.getDateVariable (thisEvent, attrValue, "[time]");
							var shortDay = reminderfox.date.getDateVariable (thisEvent, attrValue, "[shortDay]");
							var dateRef =  actualDate.getTime();
						}

						var attrElem = xmlReminders.createElement(attribute);
						attrElem.textContent = dateString;
						eventElem.appendChild(attrElem);

						var attrElem = xmlReminders.createElement(attribute + "_time");
						attrElem.textContent = ((timeString != "") ? timeString : " ");
						eventElem.appendChild(attrElem);

						var attrElem = xmlReminders.createElement(attribute + "_shortDay");
						attrElem.textContent = shortDay;
						eventElem.appendChild(attrElem);

						var attrElem = xmlReminders.createElement(attribute + "_ref");
		...
		function create_element (type, events, remindersElem) {

			for (var i = 0; i < events.length; i++) {
				var thisEvent = events[i];
				var id = thisEvent.id;

				var dateRef = thisEvent.date;
				var actualDate = dateRef;
				var actualSummary = thisEvent.summary;

				var eventElem = xmlReminders.createElement(type);
				var allDayEvent = false;
				var durationTime = 0;
		...
		for (x in localeStrings) {
			var attrElem = xmlReminders.createElement(x);
...
/**
 * Build the print/agenda menu so the user can select a template
 * @param xThis {object} this object od calling XUL menu
 */
...
//---------------------------------------------------------
	...
	var menuObject = document.getElementById(xThis.id);  // 'exportXML-popup' or 'exportXML-todo-popup'
	while (menuObject.lastChild.className != "printXMLsystem") {
		menuObject.removeChild(menuObject.lastChild);
	}

	//   next add users xsl files
	var rmfxPath = reminderfox.util.ProfD_extend('reminderfox');
	var rmFxEntries = rmfxPath.directoryEntries.QueryInterface(Components.interfaces.nsIDirectoryEnumerator);

	while (rmFxEntries.hasMoreElements()) {
		var entry = rmFxEntries.getNext();
		entry.QueryInterface(Components.interfaces.nsIFile);

		var extPos = entry.leafName.lastIndexOf('.');
		if (extPos > -1) {
			var extName = entry.leafName.substring(extPos);
			if (extName == '.xsl') {
				// add to popup menu 		<menuitem  label="Table(Example)"   value="Table(Example).xsl"
				var m1 = document.createElement("menuitem");
				m1.setAttribute("label", entry.leafName.substring(0, extPos));
				m1.setAttribute("value", entry.leafName);
				m1.setAttribute("tooltiptext", entry.leafName + " [" + new Date(entry.lastModifiedTime).toLocaleString()  + "]");
				m1.addEventListener("command", function() {reminderfox_xmlPrint(this, 'user');},false);
				menuObject.appendChild(m1);
			}
		}
	}
	return menuObject;
...
/**
 * Copy Print/View templates .XSL files from profile to user's profile directory 'reminderfox'
 * The xsl filename has to include '[example]' to be copied.
 * Files are only copied if source is newer than one stored in 'reminderfox' 
 */
...
//---------------------------------------------------------
	...
	while (prntPathEntries.hasMoreElements()) {
		...
		template.QueryInterface(Components.interfaces.nsIFile);

		var extPos = template.leafName.lastIndexOf('.');
		if (extPos > -1) {
			var extName = template.leafName.substring(extPos);
			var example = template.leafName.toLowerCase().indexOf('[example]') != -1;
			if ((extName == '.xsl') && example){
				reminderfox_copyTemplate (template);
			}
		}
	}

		function reminderfox_copyTemplate (template) {
			var rmFxDir = reminderfox.util.ProfD_extend('reminderfox');
			rmFxDir.append(template.leafName);
			var rmFxDirPath = rmFxDir.path;
			var cParentDir = rmFxDir.parent;

			var templateCopy = Components.classes["@mozilla.org/file/local;1"].createInstance(Components.interfaces.nsILocalFile);
			templateCopy.initWithPath(rmFxDirPath);

			var templateOrg = Components.classes["@mozilla.org/file/local;1"].createInstance(Components.interfaces.nsILocalFile);
			templateOrg.initWithPath(template.path);

			// copy file if DON'T exists and flag for copy is set  OR if the timestamp of the ORG is newer,
			// but set the timestamap equal to origin
			if (!templateCopy.exists()  
				|| (templateCopy.exists() && (templateCopy.lastModifiedTime < templateOrg.lastModifiedTime))) {
					templateOrg.copyTo(cParentDir, template.leafName);
					// set the timestamp of copy to org
					templateCopy.lastModifiedTime = templateOrg.lastModifiedTime
			} // copy
		};
};
