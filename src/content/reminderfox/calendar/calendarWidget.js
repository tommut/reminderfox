...
/*---
	docu.attributes[id+"_screenX"].value = +docu.attributes["screenX"].value
	  + +docu.attributes["lxX"].value;
	docu.attributes[id+"_screenY"].value = +docu.attributes["screenY"].value
	  + +docu.attributes["lxY"].value;
---*/
//	reminderfox.calendar.layout.XY ("  ++++++++++++++rmFx_mainDialogLoad    ", layout);
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//	reminderfox.calendar.layout.XY ("  ++++++++++++++.calendar.layout.Set  : ", layout);
...
//window.moveTo(document.documentElement.attributes[id+"_screenX"].value, document.documentElement.attributes[id+"_screenY"].value);
//	var X = document.documentElement.attributes[id+"_screenX"].value; if (X<0) X=0;
//	var Y = document.documentElement.attributes[id+"_screenY"].value; if (Y<0) Y=0;
//	setTimeout(function () {reminderfox.calendar.layout.Set2(X, Y)}, 0)

	document.documentElement.attributes['layout'].value = layout;
	reminderfox.calendar.layout.status = +layout;
};

	reminderfox.calendar.layout.Set2= function (X,Y) {
		window.moveTo(X,Y);
	};


reminderfox.calendar.layout.XY= function (info, layout) { // used only for debugging
//------------------------------------------------------------------------------
...
//reminderfox.util.Logger('calndrGrid', logMsg)
	...
	reminderfox.calendar.ui.selectedDayChange (oldDateNum, selectedDateNum, '### 111 ###');
};


reminderfox.calendar.ui.selectedDayChange= function (clearDateNum, setDayNum, info) {
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
//reminderfox.util.Logger('calndrGrid', logMsg)
	...
	reminderfox.calendar.ui.selectedDayChange (oldDateNum /*clear*/, selectedDateNum /*set*/, '### 222 ###');
};
...
//		return;
	...
	reminderfox.calendar.ui.selectedDayChange (oldDateNum, selectedDateNum, '### 333 ###');
}
...
//		"   View :" + j + " value:" + reminderfox.view.views.Label[j]);
	...
	var m1 = document.createElement("menuitem");
		...
		m1.setAttribute("label", reminderfox.view.tttEditor);
		m1.setAttribute("value", "vEditor");
		m1.setAttribute("tooltiptext", reminderfox.view.tttEdit);
	elem1.appendChild(m1);
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	var xpos = xThis.boxObject.x + xThis.boxObject.width * .3;
	var ypos =  xThis.boxObject.height * .9;
		...
		for(var j = 0; j < len; j++) {
			...
			for(var i = 0; i < rLen; i++) {
				...
				if //  marked as RemindUntilComplete
					((ignore == false) 
						&& (event.remindUntilCompleted && (event.remindUntilCompleted == reminderfox.consts.REMIND_UNTIL_COMPLETE_MARKED /* 2 */))) {
					var ignore = true;
