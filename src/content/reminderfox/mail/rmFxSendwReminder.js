...
//reminderfox.util.Logger('FolderListener', "  ######## sendPlus : " +  identity.fccFolder)
...
reminderfox.sendPlus.openMail = function (){
	...
	if (rmRef.length == 1) {	
		...
		// the XUL is loaded by reminderfox.sendPlus.reminder4mailList()
	}
};	
		...
		function reminderListaddItem  (n, msgId, subject, dateStr, notes) {
				...
				// RmFx date
						...
						vbox0.appendChild(hboxD);
							...
							var bText= reminderfox.string("rf.add.mail.message.date")
				...
				// RmFx notes	
						...
						vbox0.appendChild(hboxN);
							...
							var bText= reminderfox.string("rf.add.mail.message.notes")
...
reminderfox.sendPlus.getRmRef = function (msgHdr) {
	...
	// check 'reminders'
	var j=0;
	reminderfox.core.reminderFoxEvents = null;  //gWTEST SM addition ??
