...
function updateRepeatingList(currentDate, reminder, ignoreCustomTag) {
	...
	if(reminder != null && !ignoreCustomTag 
			...
			|| (reminder.recurrence.interval != null && reminder.recurrence.interval > 1) 
			|| reminder.recurrence.endDate != null )) {
		val = val + startingParens + CUSTOM + ": " + getWeekName(weekNumber) + " " + getDayName(reminderDay) + " " + reminderFoxBundle.getString("rf.options.weekof.name") + " " + monthlist.selectedItem.label + closingParens;
	} else {
		val = val + startingParens + getWeekName(weekNumber) + " " + getDayName(reminderDay) + " " + reminderFoxBundle.getString("rf.options.weekof.name") + " " + monthlist.selectedItem.label + closingParens;
	}
	items[1].label = val;
	...
	if(reminder != null && !ignoreCustomTag 
			...
			&& reminder.recurrence.interval > 1) 
			|| reminder.recurrence.endDate != null )) {
		val = val + startingParens + CUSTOM + ": " + getWeekName(weekNumber) + " " + getDayName(reminderDay) + " " + reminderFoxBundle.getString("rf.options.weekofmonth.name") + closingParens;
	} else {
		val = val + startingParens + getWeekName(weekNumber) + " " + getDayName(reminderDay) + " " + reminderFoxBundle.getString("rf.options.weekofmonth.name") + closingParens;
	}
	items[3].label = val;
...
//ALARM
	if(!editing) {
		...
		// trigrel::   TRIGGER:-PT5M		(may contain TRIGGER;RELATED=END:PT5M .. ignored that for the moment )
				var _alarmMins = reminderfox.core.getDurationAsMinutes(alarmTime)
				alarmTime = ("-PT") + _alarmMins + "M"
			}
...
function reminderFox_handleTodoDate(editing) {
	if(isDateEnabled()) {
		...
		dateItem.removeAttribute("disabled")
		dateItem = document.getElementById('datepickerAnchor');
		dateItem.removeAttribute("disabled");
		dateItem = document.getElementById('datepickerEndAnchor');
		dateItem.removeAttribute("disabled");
